...
def _buyhold_return(eq: pd.Series, initial_capital: float) -> float:
    """Simple buy-and-hold return derived from the equity curve dates."""
    if eq is None or eq.empty:
        return 0.0
    return float((eq.iloc[-1] / initial_capital) - 1)
...
def walkforward_report(
    ...
    final_cap = initial
    for w in windows:
        oos_ret = w.oos_metrics.get("cagr", 0.0)  # approximate
        trades  = w.oos_metrics.get("total_trades", 0)
        # use actual equity change if available
        final_cap += sum(
            w.oos_metrics.get("expectancy", 0.0) * w.oos_metrics.get("total_trades", 0)
            for w in windows
        ) / max(len(windows), 1)
        break  # just compute once

    # Recompute properly: chain OOS returns
    cum_equity = initial
    for w in windows:
        n_trades   = w.oos_metrics.get("total_trades", 0)
        expectancy = w.oos_metrics.get("expectancy", 0.0)
        cum_equity += n_trades * expectancy
    ...
    for i, w in enumerate(windows, 1):
        ...
        params_str = "  ".join(f"{k.replace('fvg_','').replace('atr_','').replace('_mult','')}={v}"
        ...
        lines.append(
            ...
            f"{params_str:<24}  "
            f"{_pct(oos_ret):>{col_w[4]}}  "
            f"{_f(oos_sh):>{col_w[5]}}  "
            f"{_pct(oos_dd):>{col_w[6]}}  "
            f"{n_tr:>{col_w[7]}}  "
    ...
    lines += [
        _divider("-"),
        f"  {'AVG':<{col_w[0]+col_w[1]+27}}  "
        f"{_pct(ret_sum/n_windows):>{col_w[4]}}  "
        f"{_f(sharpe_sum/n_windows):>{col_w[5]}}  "
        f"{_pct(dd_sum/n_windows):>{col_w[6]}}  "
        f"{total_trades//n_windows:>{col_w[7]}}",
        f"  {'TOTAL':<{col_w[0]+col_w[1]+27}}  "
        f"{'':>{col_w[4]}}  "
        f"{'':>{col_w[5]}}  "
        f"{'':>{col_w[6]}}  "
        f"{total_trades:>{col_w[7]}}  "
        f"{_money(cum_eq):>{col_w[8]}}",
        _divider("="),
    ]
